...
const currentPath = path.join('C:', 'Users', 'kolia', 'Downloads')
...
for (let i = 0; i < downloadFolder.length; i++) {
    ...
    const extention = utils.cleanExtension(usedFileData.ext);
    if (newFoldersArr.includes(usedFileData.base))                           //Отбрасываем только что созданные папки
        ...
        if (extention) {                                                //Проверяем, есть ли окончание
            if (!handledExts.includes(extention)) {
                console.log('We cannot detect folder for a file ext:', extention);
                ...
                entriesFoldersDictionary.forEach(entry => {
                    ...
                    if (possibleExtention.includes(extention)) {                    //Если есть, проверяем входит ли оно в архив
